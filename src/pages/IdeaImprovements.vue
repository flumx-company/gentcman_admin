...
<script>
    ...
    export default {
        ...
        methods: {
            $init() {
                this.list = this.$updateList(this.$route.meta?.ideaImprovements?.data) || [];
        ...
        computed: {
            $headers: () => ['id',  'created_at', 'status', 'email',  'improvement',  'user_name', 'message',   'action'],
            $keyName: () => 'id',
            $standartPagination: () => false,
            texts(){
                return this.getTexts('IdeaImprovementsTexts')
            },
            $selectedNameFieldForDelete: () => 'name',
            $params: () => ({
                showCheck: true
            }),
            $request: () => $api.ideaImprovementsRequest
        }
    }
</script>
