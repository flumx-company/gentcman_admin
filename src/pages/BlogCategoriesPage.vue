...
<script>
    ...
    export default {
        ...
        data:() => ({
            form: {
                name: {
                    value: '',
                    label: 'Имя',
                    placeholder: 'Как создают кошельки',
                    type: 'text',
        ...
        methods: {
            $init() {
                this.list = this.$route.meta?.blogCategories?.data || [];
            },
        ...
        computed: {
            ...
            texts(){
                return this.getTexts('BlogCategoriesTexts')
            },
            $selectedNameFieldForDelete: () => 'name',
            $params: () => ({
                showCheck: true
            }),
            $request: () => $api.blogCategoriesRequest
        }
    }
</script>
