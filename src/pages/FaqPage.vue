    ...
    export default {
        ...
        data:() => ({
            ...
            statuses: [
                ...
                { name: "В процессе исп", id: 'in the process', color: '#ffa50073' },
                { name: "Закрыто - не успешно", id: 'closed - problem not solved', color: '#ff00008f' },
                { name: "Закрыто - успешно", id: 'closed - successful', color: '#008000a1' },
            ],
            messageTemplates: []
        }),
        methods: {
            $init() {
                this.list = this.$updateList(this.$route.meta?.faq?.data) || [];
        ...
        computed: {
            $headers: () => ['id',  'created_at', 'status', 'email',  'theme',  'content', 'message',   'action'],
            $keyName: () => 'id',
            $standartPagination: () => false,
            texts(){
                return this.getTexts('FaqTexts')
            },
            $selectedNameFieldForDelete: () => 'name',
            $params: () => ({
                showCheck: true
            }),
            $request: () => $api.faqRequest
        }
    }
</script>
